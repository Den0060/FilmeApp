...
#  Nur relevant wenn Firebase verfügbar ist – sonst komplett übersprungen.
...
# Beim Start einmalig prüfen – aber nur wenn Firebase überhaupt konfiguriert ist.
# Im reinen Lokalmodus ist der Online-Status irrelevant.
OFFLINE = (not _internet_verfuegbar()) if FIREBASE_VERFUEGBAR else False
...
def firestore_push_film(film_id: int):
    ...
    Im Lokalmodus (kein Firebase) sofort No-op.
    """
...
#  Taucht nur auf wenn keine filmvault-firestore.json gefunden wurde.
#  Erklärt dem User was zu tun ist – kein Datei-Browser, kein
#  kompliziertes Setup: JSON neben die .exe legen, neu starten, fertig.
# ──────────────────────────────────────────────────────────────
...
class FirebaseVerbindenDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        # Erklärungstext
        anleitung = (
            "Damit FilmVault deine Filme geräteübergreifend synchronisiert,\n"
            "wird eine Firebase Service-Account-Datei benötigt.\n\n"
            "So geht's:\n\n"
            "  1.  Firebase Console öffnen → Projekteinstellungen\n"
            "  2.  Reiter „Dienstkonten → „Neuen privaten Schlüssel generieren\n"
            "  3.  Die heruntergeladene JSON-Datei umbenennen in:\n\n"
            "          filmvault-firestore.json\n\n"
            "  4.  Datei in denselben Ordner legen wie diese\n"
            "      filmvault.exe  bzw.  filmvault.py\n\n"
            "  5.  FilmVault neu starten – die Verbindung wird\n"
            "      dann automatisch erkannt.\n\n"
            "Ohne diese Datei läuft FilmVault weiterhin lokal\n"
            "und speichert alles nur auf diesem Gerät.\n"
            "Sollten Sie die Anwendung bisher nur lokal genutzt haben,\n"
            "sollten Sie prüfen, ob Ihre Datenbank in Firestore übertragen wurde\n"
            "ggf. manuell übertragen (jeden Film einzelnd erneut speichern)"
        )
        ...
        pfad = os.path.join(_base(), "filmvault-firestore.json")
        tk.Label(self, text=f"Erwarteter Pfad:\n{pfad}",
...
class FilmApp(tk.Tk):
    def __init__(self):
        ...
        # Offline-Banner und periodische Checks nur wenn Firebase aktiv ist
        if FIREBASE_VERFUEGBAR:
    ...
    def _offline_geblockt(self) -> bool:
        """
        Gibt True zurück und zeigt Meldung wenn offline.
        Im Lokalmodus (kein Firebase) ist man nie geblockt – man schreibt immer lokal.
        """
        if FIREBASE_VERFUEGBAR and self._offline:
    ...
    def _build_ui(self):
        ...
        # Firebase-Button nur anzeigen wenn keine JSON gefunden wurde –
        # im verbundenen Modus braucht der User das nicht zu sehen
        if not FIREBASE_VERFUEGBAR:
            tk.Frame(sidebar, bg=BORDER, height=1).pack(fill="x", pady=(16, 0))
            tk.Button(
                sidebar,
                text="☁  Firebase verbinden",
                bg=PANEL, fg=MUTED,
