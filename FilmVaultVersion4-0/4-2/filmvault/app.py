...
class FilmApp(tk.Tk):
    ...
    def _sortiere(self, tree, spalte, frame):
        ...
        def sort_key(x):
            ...
            if wert in ("", "–", "–min"):
                return (1, 0, "")
            if wert.startswith("✅ ") and wert.count(".") == 2:
                try:
                    datum = wert[2:].strip()  # "TT.MM.JJJJ"
                    tag_, mon_, jahr_ = datum.split(".")
                    return (0, 0, f"{jahr_}-{mon_}-{tag_}")
                except Exception:
                    pass
            try:
    ...
    def _fill_tree(self, tree, rows, frame=None):
        ...
        for r in rows:
            ...
            fid, titel, jahr, bew, genre, gesehen, laufzeit, imdb_bew, imdb_id, poster_url, gesehen_am, updated_at, notiz = r
        ...
        if frame is not None and frame._aktive_sortierung is not None:
            ...
            frame._sort_state[spalte] = not absteigend

            self._sortiere(tree, spalte, frame)
    ...
    def _film_speichern(self, titel, jahr, bewertung, genre, laufzeit, imdb_bewertung, imdb_id, poster_url, notiz=None):
        db_hinzufuegen(titel, jahr, bewertung, genre, laufzeit, imdb_bewertung, imdb_id, poster_url, notiz=notiz)
        self.aktualisieren()
    ...
    def film_bearbeiten_dialog(self, tree):
        ...
        _, titel, jahr, bew, genre, _, laufzeit, imdb_bew, imdb_id, poster_url, _, _, notiz = row

        def save(t, j, b, g, lz, ib, iid, purl, notiz=None):
            db_bearbeiten(fid, t, j, b, g, lz, ib, iid, purl, notiz=notiz)
            self.aktualisieren()
