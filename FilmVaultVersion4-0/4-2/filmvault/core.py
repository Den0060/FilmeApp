...
_FELDER = "id, titel, jahr, bewertung, genre, gesehen, laufzeit, imdb_bewertung, imdb_id, poster_url, gesehen_am, updated_at, notiz"
...
def firestore_push_film(film_id: int, touch_meta: bool = True):
    ...
    fid, titel, jahr, bew, genre, gesehen, laufzeit, imdb_bew, imdb_id, poster_url, gesehen_am, updated_at, notiz = row
    payload = {
        "id":             fid,
        "titel":          titel,
        "jahr":           jahr,
            ...
            cur.execute("""
                INSERT INTO filme
                    (id, titel, jahr, bewertung, genre, gesehen, laufzeit, imdb_bewertung,
                     imdb_id, poster_url, gesehen_am, updated_at, notiz)
                VALUES
                    (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def _alle_felder():
    return "id, titel, jahr, bewertung, genre, gesehen, laufzeit, imdb_bewertung, imdb_id, poster_url, gesehen_am, updated_at, notiz"
...
def db_hinzufuegen(titel, jahr, bewertung, genre, laufzeit=None, imdb_bewertung=None, imdb_id=None, poster_url=None, notiz=None):
    con = sqlite3.connect(DB_FILE)
    cur = con.cursor()
    updated_at = _now_iso()
    cur.execute(
        """INSERT INTO filme (titel, jahr, bewertung, genre, laufzeit, imdb_bewertung, imdb_id, poster_url,
                              gesehen, gesehen_am, updated_at, notiz)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, NULL, ?, ?)""",
        (titel, jahr, bewertung, genre, laufzeit, imdb_bewertung, imdb_id, poster_url, updated_at, notiz)
    )
    fid = cur.lastrowid
    con.commit()
    con.close()
...
def db_bearbeiten(film_id, titel, jahr, bewertung, genre, laufzeit=None, imdb_bewertung=None, imdb_id=None, poster_url=None, notiz=None):
    con = sqlite3.connect(DB_FILE)
    cur = con.cursor()
    updated_at = _now_iso()
    cur.execute(
        """UPDATE filme SET titel=?, jahr=?, bewertung=?, genre=?,
           laufzeit=?, imdb_bewertung=?, imdb_id=?, poster_url=?, updated_at=?, notiz=? WHERE id=?""",
        (titel, jahr, bewertung, genre, laufzeit, imdb_bewertung, imdb_id, poster_url, updated_at, notiz, film_id)
    )
    con.commit()
    con.close()
    if _cloud_scope_aktiv():
