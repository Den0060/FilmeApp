...
class FilmDialog(tk.Toplevel):
    ...
    def _build(self, titel, prefill):
        ...
        self._label(left, "Notiz (optional)")
        self.e_notiz = tk.Text(left, bg="#0d0d14", fg=TEXT, insertbackground=TEXT,
                               font=("Segoe UI", 10), bd=0, relief="flat",
                               highlightthickness=1, highlightbackground=BORDER,
                               highlightcolor=ACCENT, height=4, wrap="word")
        self.e_notiz.pack(fill="x", pady=(0, 4))
        ...
        if prefill:
            titel_str, jahr_str, bew_str, genre_str, laufzeit_str, imdb_str, imdb_id, poster_url, notiz_str = prefill
            ...
            self.e_notiz.insert("1.0", notiz_str)
    ...
    def _details_fertig(self, details, movie_id):
        ...
        if poster_url:
            ...
            self._poster_platzhalter()
            self._poster_ref = None # alte Referenz freigeben
    ...
    def _speichern(self):
        ...
        notiz = self.e_notiz.get("1.0", "end").strip() or None

        self.callback(titel, jahr, bewertung, genre, laufzeit, imdb_bewertung, self._imdb_id, self._poster_url, notiz)
        self.destroy()
