    ...
    def __init__(self, parent, auth: dict, preset_group: str = "", preset_mode: str = "user"):
        ...
        def _entry():
            return tk.Entry(
                ...
                disabledbackground="#3a3a4a",
                disabledforeground="#a0a0aa",
                readonlybackground="#252535",
                font=("Segoe UI", 11),
                bd=0,
                relief="flat",
        ...
        self.join_mode_var = tk.StringVar(value="code_open")
        self._join_mode_frame = tk.Frame(form, bg=CARD)
        self._join_mode_frame.pack(fill="x", pady=(8, 0))
        tk.Label(self._join_mode_frame, text="Beitritt bei neuer Gruppe", bg=CARD, fg=MUTED, font=("Segoe UI", 9, "bold")).pack(anchor="w")
        tk.Radiobutton(
            self._join_mode_frame, text="Jeder mit Code kann direkt beitreten", variable=self.join_mode_var, value="code_open",
            bg=CARD, fg=TEXT, selectcolor=CARD, activebackground=CARD, activeforeground=TEXT,
            font=("Segoe UI", 9)
        ).pack(anchor="w")
        tk.Radiobutton(
            self._join_mode_frame, text="Mit Code, aber erst nach Bestätigung", variable=self.join_mode_var, value="approval",
            bg=CARD, fg=TEXT, selectcolor=CARD, activebackground=CARD, activeforeground=TEXT,
            font=("Segoe UI", 9)
        ).pack(anchor="w", pady=(2, 0))
        ...
        try:
            ...
            if mode == "user":
                ...
                if action == "create":
                    ...
                    group_data = _gruppe_per_code_laden(
                        auth.get("id_token") or "",
                        group_code,
                        auth.get("uid") or "",
                        auth.get("email") or "",
                ...
                auth["group_role"] = group_data.get("group_role") or "member"
                auth["group_status"] = group_data.get("group_status") or "active"
