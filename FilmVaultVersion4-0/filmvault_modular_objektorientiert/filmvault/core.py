...
def _gruppe_per_code_laden(id_token: str, code: str) -> dict:
    """Lädt die Gruppendaten zu einem Code."""
    code_key = _gruppen_code_normalisieren(code)
    if len(code_key) < 6:
        raise ValueError("Bitte einen gültigen Gruppencode eingeben.")
    doc = _firestore_request_with_token("GET", f"groupCodes/{code_key}", id_token)
    if not doc:
...
def _gruppe_mit_code_erstellen(id_token: str, uid: str, email: str, group_name: str) -> dict:
    """Erstellt eine Gruppe und speichert einen einmaligen Beitrittscode."""
